  ...
  <div v-if="userDetails !== null">
    ...
    <v-row align="center" justify="center">
      <v-col cols="11" lg="4" md="4" sm="4">
        ...
        <v-text-field
          ...
          color="light-blue darken-4"
          v-on:keypress="
            validateMaxChars($event, userData.details.secondLastName)
          "
        ></v-text-field>
        <v-text-field
    ...
    <v-row align="center" justify="center">
      <v-col class="text-center">
        ...
        <v-btn color="primary" @click="checkingValidForm" :loading="loading">
          {{ $t("common.Save") }}
        </v-btn>
        <snackbar
          @close="closeSnackbar"
...
export default {
  ...
  methods: {
    ...
    resetValues() {
      if (this.userDetails !== null) {
        ...
        if (
          this.userData.details.birthdate !== null &&
          this.userData.details.birthdate !== ""
        ) {
    ...
    dataChanged() {
      if (
        this.userData.details.address !== this.userDetails.details.address ||
        this.birthdateChanged() ||
        this.countryChanged(
          this.userData.details.country,
          this.userDetails.details.country
        ) ||
        this.userData.details.firstName !==
          this.userDetails.details.firstName ||
        this.userData.details.lastName !== this.userDetails.details.lastName ||
        this.userData.details.middleName !==
          this.userDetails.details.middleName ||
        this.userData.details.phone !== this.userDetails.details.phone ||
        this.userData.details.secondLastName !==
          this.userDetails.details.secondLastName ||
        this.userData.email !== this.userDetails.email
      ) {
        return true;
      }
    ...
    birthdateChanged() {
      if (
        this.birthdate !== null &&
        this.userDetails.details.birthdate !== null &&
        this.birthdate !== this.userDetails.details.birthdate.split("T")[0]
      ) {
        return true;
      } else if (
        (this.birthdate !== null &&
          this.userDetails.details.birthdate === null) ||
        (this.birthdate === null && this.userDetails.details.birthdate !== null)
      ) {
        return true;
      }
      return false;
    ...
    countryChanged(newCountry, currentCountry) {
      if (
        (currentCountry === undefined && newCountry !== undefined) ||
        (currentCountry === null && newCountry !== null)
      ) {
        return true;
      } else if (
        ...
        currentCountry !== null &&
        newCountry !== null &&
        currentCountry.name !== newCountry.name
      ) {
        return true;
      }
      return false;
    ...
    restrictChars(event) {
      if (
        event.charCode < 48 ||
        event.charCode > 57 ||
        (this.userData.details.phone !== null &&
          this.userData.details.phone.length === 15)
      ) {
  ...
  computed: {
    ...
    firstNameErrors() {
      ...
      !this.$v.userData.details.firstName.required &&
    ...
    lastNameErrors() {
      ...
      !this.$v.userData.details.lastName.required &&
