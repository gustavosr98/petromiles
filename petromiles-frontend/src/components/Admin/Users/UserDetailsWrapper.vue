...
export default {
  ...
  async mounted() {
    try {
      this.conversion = await this.$http.get(
        `user/points/conversion?id=${this.user.idUserClient}`
      );
      this.membership = await this.$http.get(
        `suscription/actual?id=${this.user.idUserClient}`
      );
      this.userBankAccounts = await this.$http.get(
        `bank-account?id=${this.user.idUserClient}`
      );
      const userInformation = await this.$http.get(
        `user/${this.user.idUserClient}/CLIENT`
      );
      this.transactions = await this.$http.get(
        `transaction?id=${this.user.idUserClient}`
      );
